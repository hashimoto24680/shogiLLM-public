...
LLMを用いて将棋AIの指し手を自然言語で解説する研究デモです。

> 🏆 本研究は、情報処理学会 **インタラクション2026** にて **インタラクティブ発表賞（PC推薦）** を受賞しました。

## 30秒でわかる概要

将棋AIは人間トッププロを超える棋力を持つ一方で、出力は「評価値」と「読み筋」だけで、有段者でないと指し手の意図を理解するのが困難という課題があります。本研究では、**初心者模倣AIと高棋力AIの対局シミュレーション**で「不利になる手順」を可視化し、**局面の静的・動的特徴を言語化**したうえで LLM に渡すことで、初心者にも理解しやすい解説文を自動生成します。

プロ棋士による評価実験で、対局シミュレーションと局面特徴を統合した提案手法が、LLM単体や片方のみの構成より具体手順を伴う論理的な解説生成に成功することを確認しました。
...
本システムは3段階のパイプラインで将棋の解説文を生成します。納得感のある解説には「ユーザーが考えそうな手」「それが不利になる手順」「各変化の状況の言語による説明」の3点が必要であり、これらを順に補う構成になっています。

### Step 1: 対局シミュレーション

初心者模倣AI（**Maia2**）と高棋力AI（**YaneuraOu**）の対局を、現局面から終端まで進めて取得します。これにより、初心者が指しそうな手とそれが不利になっていく具体的な変化手順を、形勢差が明確になる末端局面まで可視化します。

### Step 2: 局面特徴の言語化

各局面から以下の特徴を抽出し、LLMが理解可能な自然言語へ変換します。
- **静的特徴**: 各マスの駒の利き、囲い（美濃・穴熊など）、戦型、駒の働き
- **動的特徴**: シミュレーション前後の静的特徴の差分（駒損得、評価値の変化など）

LLMは盤面の空間情報を扱うのが苦手なため、これにより非合法手や盤上にない駒への言及を抑制します。

### Step 3: LLM による解説文生成

上記をすべて統合したプロンプトを LLM に与えて解説文を生成します。プロンプトには以下が含まれます。
- 対局シミュレーション結果（変化手順と末端局面の動的特徴）
- 対象局面の静的特徴
- 最善手順、解説の文体例、構成の指示

なお、本研究の評価実験（プロ棋士・谷合廣紀氏による評価）では、対局シミュレーションと局面特徴を**両方使った構成**が、いずれか単独・LLM単体のいずれよりも具体手順を伴う論理的な解説生成に成功することを確認しています。
...
```
### 2. 環境変数の設定
`.env.example` をコピーして `.env` を作成し、`OPENAI_API_KEY` を設定してください。
```bash
cp .env.example .env
